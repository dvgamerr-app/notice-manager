const mintify = (text) => {
  return text.includes('hrs')
    ? text.replace(/ hr(|s) /gi, ':').replace(/min(|s)/gi, 'hr')
    : text
}

module.exports = (e, data) => {
  if (!data.length) {
    return 'ไม่มีข้อมูล ผู้เข้าร่วมจัดอันดับ ครับผม!!'
  }
  data = data.sort((a, b) =>
    a.stats.total_seconds <= b.stats.total_seconds ? 1 : -1
  )
  const contents = [
    {
      type: 'box',
      ...
      contents: [
        ...
        {
          type: 'image',
          url: 'https://wakatime.com/static/img/wakatime.svg',
          gravity: 'top',
          aspectMode: 'fit',
          aspectRatio: '1:1',
          size: '25px',
