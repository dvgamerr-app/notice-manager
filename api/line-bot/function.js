...
module.exports = async (req, h) => {
  try {
    ...
    const botRoom = await LineBotRoom.findOne({ _id: '607a6a21209b8a40682f68a8' })
    if (!botRoom || !botRoom.variable) { return { OK: false } }

    if (req.method === 'get') {
      return botRoom.variable.map(({ userId, rank, data }) => {
      ...
      for (let i = 0; i < botRoom.variable.length; i++) {
        ...
        await LineBotUser.updateMany({ botname: bot, roomname: room, userId: row.userId })
        new LineBotUser({ botname: bot, roomname: room, userId: row.userId, name: user.display_name }).save()
      ...
      botRoom.variable = botRoom.variable.sort((a, b) => a.data.wakaStats.total_seconds <= b.data.wakaStats.total_seconds ? 1 : -1)
      for (let i = 0; i < botRoom.variable.length; i++) {
        const row = botRoom.variable[i]
        if (row.rank !== i + 1) {
      ...
      await LineBotRoom.updateOne({ _id: botRoom._id }, { $set: { variable: botRoom.variable } })
