...
module.exports = async (req, reply) => {
  if (!req.body || !req.body.type) {
    return reply.status(400).send({
      statusCode: 400,
      error: 'Bad Request',
      message: 'Not found type LINE Bot sender format.'
    })
  }
  ...
  if (
    !/^[RUC]{1}/g.test(userTo) &&
    (!/[0-9a-f]*/.test(userTo) || userTo.length !== 32)
  ) {
    ...
    if (!room || !room.active) {
      return reply.status(400).send({
        statusCode: 400,
        error: 'Bad Request',
        message: ' Bot name or Room name not found.'
      })
    }
    userTo = room.userId
  }

  const type = userTo.startsWith('R')
    ? 'room'
    : userTo.startsWith('C')
      ? 'group'
      : userTo.startsWith('U')
  ...
  try {
    ...
    await LineOutbound.updateOne(
      { _id: outbound._id },
      { $set: { sended: false, error: ex.message || ex.toString() } }
    )
    ...
    return reply.status(400).send({
      ...
      error: 'Bad Request',
      message:
        'Incorrect body format, https://developers.line.biz/en/docs/messaging-api/message-types/#text-messages'
    })
  }
