...
const leaveBotRoom = async (botname, id, type) => {
  ...
  return notice
    .get('LineBotRoom')
    .updateOne({ botname, id, type }, { $set: { active: false } })
}
const renameBotRoom = async (botname, id, type, name) => {
  ...
  return notice
    .get('LineBotRoom')
    .updateOne({ botname, id, type }, { $set: { name } })
}
...
module.exports = {
  onEvents: {
    join: async (botname, event) => {
      ...
      await monitorLINE(
        `Bot your join in ${event.source.type} (${getID(event)}).`
      )
      return 'มาแล้วๆ'
    },
    leave: async (botname, event) => {
      ...
      await monitorLINE(
        `Bot your leave from ${event.source.type} (${getID(event)}).`
      )
    }
  },
  onCommands: {
    ...
    room: async (botname, args, event) => {
      ...
      if (await verifyRoom(botname, args[0])) {
        return `\`${args[0]}\` ใช้แล้ว`
      }
    ...
    api: async (botname, args, event) => {
      const room = await getRoom(botname, getID(event), event.source.type)
      const url = `${api}/${botname}/${
        room.name && room.active ? room.name : getID(event)
      }`
      const curl =
        '```\n' +
