<template>
  <b-row v-if="!$store.state.wait && item">
    ...
    <b-col v-if="!$route.query.view" cols="3" class="pl-0 pt-3 mb-1">
      <b-button :to="`/liff/${item.type}/${item.value}/new`" variant="info" block class="btn-new">
        <fa icon="plus" /> <span>Join</span>
      </b-button>
    </b-col>
    <b-col cols="12" class="py-3">
      <lazy-liff-item-drop v-for="e in room" :key="e.$id" @delete="remove(e)">
        <nuxt-link :to="`/liff/${item.type}/${item.value}/${e.value}`" class="d-flex align-items-center list-item py-3 border-bottom">
...
export default {
  ...
  async asyncData ({ $api, $router, store, env, params }) {
    const { status, data: room } = await $api.request(`GET /notify/${params.service}/room`, {
      headers: { 'x-user-liff': store.state.profile.userId }
    })
    if (status !== 200) { await $router.replace('/liff/notify') }
  ...
  created () {
    const [data] = this.$store.state.lineNotify.filter(e => e.value === this.service)
    this.item = data
  },
  methods: {
    getUsage (stats) {
