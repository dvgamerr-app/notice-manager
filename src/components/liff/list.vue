  ...
  <b-row v-if="!$store.state.wait">
      ...
      <b-row v-if="!err">
        ...
        <b-col v-if="type != 'all'" cols="3" class="pl-0 pt-3 mb-1">
          <b-button
            ...
            class="btn-new"
          >
            <fa icon="plus" class="d-sm-inline-block d-none" />
            <span>Create</span>
          </b-button>
        </b-col>
      </b-row>
    ...
    <b-col sm="12" class="pb-3">
      <nuxt-link
        ...
        :event="''"
        to="#"
        class="d-block list-item empty py-3 border-bottom"
      >
        {{ err || 'Empty.' }}
      </nuxt-link>
      <lazy-liff-item-drop
      ...
      >
        <nuxt-link
          :to="`/liff/${e.type}/${e.value}${type == 'all' ? `?view=1` : ''}`"
          class="d-flex align-items-center list-item py-3"
        >
          <div class="icon px-1">
          ...
          <nuxt-link
            v-show="type != 'all'"
            :to="`/liff/${e.type}/${e.value}/edit`"
            class="py-3 px-4 config"
          >
...
export default {
  ...
  computed: {
    ...
    countItem () {
      ...
      return (
        (this.getTypeBot ? lineBot.length : 0) +
        (this.getTypeNotify ? lineNotify.length : 0)
      )
    }
  },
  methods: {
    getItem () {
      const { lineNotify, lineBot } = this.$store.state
      let data = []
      if (this.type === 'all' || this.type === 'notify') {
        data = data.concat(...lineNotify)
      }
      if (this.type === 'all' || this.type === 'bot') {
        data = data.concat(...lineBot)
      }
      return data
        .filter((e) => {
          return (
            new RegExp(this.search, 'ig').test(e.text) ||
            new RegExp(this.search, 'ig').test(e.value)
          )
        })
