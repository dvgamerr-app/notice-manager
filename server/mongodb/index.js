...
const { mConn, mMapping } = {
  mConn: async (server, dbname, username, password) => {
    ...
    const MONGODB_URI = getEnv('MONGODB_URI') || `mongodb://${MONGODB_ACCOUNT}${MONGODB_SERVER}/${dbname}?authMode=scram-sha1${IsAdmin ? '&authSource=admin' : ''}`
  ...
  mMapping: (table, force = false) => {
    ...
    if (typeof table.schema !== 'object') { throw new TypeError('mongodb schema is not object.') }
